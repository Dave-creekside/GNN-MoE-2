...
# -*- coding: utf-8 -*-
"""
sweep_geometric_loss_weights.py

Fine-grained sweep of geometric loss component weights for the GCL system.
Tests different combinations of orthogonality, rotation efficiency, and specialization weights.
"""
import subprocess
import csv
import os
...
PARAMETER_BEING_SWEPT = "geometric_loss_weights"

# Fine-grained weight combinations
# We'll test various combinations while keeping total weight reasonable
WEIGHT_COMBINATIONS = [
    # Format: (ortho, efficiency, specialization, name)
    (0.1, 0.1, 0.1, "low_all"),
    (0.2, 0.1, 0.1, "low_reg"),
    (0.3, 0.2, 0.2, "balanced_light"),
    (0.5, 0.2, 0.3, "default"),  # Current optimal
    (0.7, 0.2, 0.3, "high_ortho"),
    (0.5, 0.4, 0.3, "high_efficiency"),
    (0.5, 0.2, 0.5, "high_specialization"),
    (0.8, 0.4, 0.4, "high_all"),
    (1.0, 0.2, 0.3, "max_ortho"),
    (0.5, 0.6, 0.3, "max_efficiency"),
    (0.5, 0.2, 0.8, "max_specialization"),
    (0.3, 0.3, 0.3, "equal_weights"),
    (0.6, 0.3, 0.6, "ortho_spec_focus"),
    (0.4, 0.5, 0.4, "efficiency_focus"),
    (0.2, 0.2, 0.2, "minimal_reg")
]

SWEEP_VALUES = WEIGHT_COMBINATIONS
...
baseline_config = {
    ...
    'geometric_orthogonality_weight': [0.5],  # Will be overridden
    'geometric_rotation_efficiency_weight': [0.2],  # Will be overridden
    'geometric_specialization_weight': [0.3],  # Will be overridden
...
csv_fieldnames = all_possible_param_names + [
    'run_name', 'weight_combination', 'total_loss_weight', 'total_params_str', 'best_eval_loss',
...
print(f"🚀 Starting fine-grained sweep for '{PARAMETER_BEING_SWEPT}' with {len(SWEEP_VALUES)} combinations")
print(f"⚖️ Testing different loss component weight balances")
print(f"🎯 Finding optimal balance for orthogonality, efficiency, and specialization")

# --- Main Sweep Loop ---
for i, (ortho_weight, eff_weight, spec_weight, weight_name) in enumerate(SWEEP_VALUES):
    cleanup_memory()  # Clean memory before each run
    
    current_run_params = {key: val[0] for key, val in baseline_config.items()}
    current_run_params['geometric_orthogonality_weight'] = ortho_weight
    current_run_params['geometric_rotation_efficiency_weight'] = eff_weight
    current_run_params['geometric_specialization_weight'] = spec_weight

    run_name_parts = [f"sweep_{PARAMETER_BEING_SWEPT}_{timestamp}_run{i+1:02d}"]
    run_name_parts.append(f"{weight_name}")
    run_name_parts.append(f"o{ortho_weight:.1f}e{eff_weight:.1f}s{spec_weight:.1f}")
    run_name = "_".join(run_name_parts)

    total_weight = ortho_weight + eff_weight + spec_weight

    print(f"\n{'='*60}")
    print(f"⚖️ Running {i+1}/{len(SWEEP_VALUES)}: {weight_name}")
    print(f"📊 Weights: Ortho={ortho_weight:.1f}, Eff={eff_weight:.1f}, Spec={spec_weight:.1f}")
    print(f"📈 Total regularization weight: {total_weight:.1f}")
    ...
    result_row['weight_combination'] = weight_name
    result_row['total_loss_weight'] = total_weight
    ...
    try:
        ...
        if return_code != 0:
            ...
            print(f"📊 {weight_name} → Loss: {result_row.get('best_eval_loss', 'N/A')}")
...
print(f"💡 Tip: Plot 'weight_combination' vs 'best_eval_loss' to find optimal balance")
print(f"⚖️ Look for the combination that best balances all loss components!")
print(f"🎯 This will optimize your revolutionary GCL system's loss function!")
