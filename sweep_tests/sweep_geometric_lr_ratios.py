...
# -*- coding: utf-8 -*-
"""
sweep_geometric_lr_ratios.py

Fine-grained sweep of geometric:expert learning rate ratios for the GCL system.
This sweeps the ratio between geometric_learning_rate and geometric_expert_learning_rate.
"""
import subprocess
import csv
import os
...
PARAMETER_BEING_SWEPT = "geometric_lr_ratio"

# Fine-grained ratios from 5:1 to 50:1
BASE_EXPERT_LR = 0.0001
RATIO_VALUES = np.linspace(5, 50, 20)  # 20 fine-grained steps from 5:1 to 50:1
SWEEP_VALUES = [(ratio, BASE_EXPERT_LR * ratio, BASE_EXPERT_LR) for ratio in RATIO_VALUES]

# Baseline values optimized for GCL system with wikitext-2
baseline_config = {
    'training_mode': ['geometric'],
    'geometric_enabled': [True],
    'geometric_learning_rate': [0.001],  # Will be overridden by sweep
    'geometric_expert_learning_rate': [0.0001],  # Will be overridden by sweep
    ...
    'num_experts': [2],
    'batch_size': [8],
    'learning_rate': [0.0001], # This is the expert LR
    'dropout_rate': [0.1],
    'epochs': [5],
...
csv_fieldnames = all_possible_param_names + [
    'run_name', 'lr_ratio', 'total_params_str', 'best_eval_loss',
...
print(f"🚀 Starting fine-grained sweep for '{PARAMETER_BEING_SWEPT}' with {len(SWEEP_VALUES)} ratios")
print(f"📊 Ratio range: {RATIO_VALUES[0]:.1f}:1 to {RATIO_VALUES[-1]:.1f}:1")
print(f"🎯 Base expert LR: {BASE_EXPERT_LR}")

# --- Main Sweep Loop ---
for i, (ratio, geo_lr, exp_lr) in enumerate(SWEEP_VALUES):
    cleanup_memory()  # Clean memory before each run
    
    current_run_params = {key: val[0] for key, val in baseline_config.items()}
    current_run_params['geometric_learning_rate'] = geo_lr
    current_run_params['geometric_expert_learning_rate'] = exp_lr

    run_name_parts = [f"sweep_{PARAMETER_BEING_SWEPT}_{timestamp}_run{i+1:02d}"]
    run_name_parts.append(f"ratio{ratio:.1f}to1")
    run_name_parts.append(f"geoLR{geo_lr:.0e}")
    run_name = "_".join(run_name_parts)

    print(f"\n{'='*60}")
    print(f"🧪 Running {i+1}/{len(SWEEP_VALUES)}: Ratio {ratio:.1f}:1")
    print(f"📈 Geometric LR: {geo_lr:.6f}, Expert LR: {exp_lr:.6f}")
    ...
    result_row['lr_ratio'] = f"{ratio:.1f}:1"
    ...
    try:
        ...
        if return_code != 0:
            ...
            print(f"📊 Ratio {ratio:.1f}:1 → Loss: {result_row.get('best_eval_loss', 'N/A')}")
...
print(f"💡 Tip: Plot 'lr_ratio' vs 'best_eval_loss' to find the optimal ratio")
print(f"🎯 Look for the ratio that minimizes loss in your revolutionary GCL system!")
