...
Sweeps the 'embed_dim' hyperparameter for the GNN-MoE model.
"""
import subprocess
import csv
import os
...
PARAMETER_BEING_SWEPT = "embed_dim"
SWEEP_VALUES = [256, 384, 512]  # Values to test for embed_dim

# Baseline values (used when a parameter is not being swept)
# Note: Values are in lists for consistency with the original sweep script's structure,
# even though only one value is used from baseline for non-swept params.
baseline_config = {
    'embed_dim': [384],  # This will be overridden by SWEEP_VALUES
    'num_layers': [4],
    'num_experts': [4],
    'batch_size': [64],
    'learning_rate': [3e-4],
...
# All keys from baseline_config will be columns.
...
for i, sweep_value in enumerate(SWEEP_VALUES):
    current_run_params = {key: val[0] for key, val in baseline_config.items()} # Get single values from baseline
    current_run_params[PARAMETER_BEING_SWEPT] = sweep_value # Set the swept parameter

    # Construct a descriptive run_name
    run_name_parts = [f"sweep_{PARAMETER_BEING_SWEPT}_{timestamp}_run{i+1}"]
    # Add the swept parameter to the run_name for quick identification
    ...
    command.append("checkpoints_sweep_runs") # Base directory for sweep checkpoints
